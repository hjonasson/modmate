...
function getOutputPath(absRoot, outputDir) {
  ...
  const absOutputDir = outputDir.startsWith("/") ? outputDir : node_path.resolve(absRoot, outputDir);
...
function isLegalTransformTarget(importer) {
  ...
  const ext = node_path.extname(REG_VALID_EXTENSION.test(importer) ? importer : cleanUrl(importer)).slice(1);
  ...
  return {
    ...
    async buildEnd() {
      ...
      if (dynImport.size > 0) {
        if (isDynamicModuleLive) {
          dynImport.forEach((file) => {
            const rel = node_path.relative(absoluteRoot, file);
      ...
      let result = [
        ...
        ...[...FileMarker.deadFiles].map((fullPath) => `    ./${node_path.relative(absoluteRoot, fullPath)}`)
      ];
      ...
      if (output) {
        ...
        if (outputFile) {
          await node_fs.promises.writeFile(outputFile, result.join("\n"));
